  ...
  return (() => {
    var _ref = _asyncToGenerator(function* (request, h) {
      try {
        ...
        const userAlreadyHasTwoAuth = yield (0, _UserHasAnyTwoFactorAuth2.default)({ username, getUserU2F: config.twoFactor.u2f.getUserU2F, getUserTOTP: config.twoFactor.totp.getUserTOTP });
        if (userAlreadyHasTwoAuth) {
          let userU2F = yield config.twoFactor.u2f.getUserU2F({ username });
          let authRequest = _u2f2.default.request(config.twoFactor.u2f.appId, userU2F.keyHandle);
          yield config.twoFactor.u2f.upsertUserU2F({ username, u2f: { authRequest } });
          return authRequest;
        } else {
