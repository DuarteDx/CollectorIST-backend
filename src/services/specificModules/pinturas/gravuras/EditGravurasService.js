...
export default async (token, assetId, newGravurasModule) => {
  return jwt.verify(token, 'secretKey', async (err, authData) => {
    if (err) {
      ...
      console.log(time + ' POST -> Edit asset gravuras module')
      ...
      await db.collection('assets').updateOne(
        ...
        {
          $set: { 'gravuras': newGravurasModule },
      ...
      var log = {
        time: today,
        action: 'Edit asset gravuras module',
        objectId: assetId,
      ...
      return 'Edited asset gravuras module with _id: ' + assetId
    }
  })
}
